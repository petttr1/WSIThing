...
class NNClassifier:
    def __init__(self, name, input_shape=(128, 128, 3), path_to_file=None):
        ...
        self.lr = 1e-6
        self.net.compile(
            optimizer=Adam(lr=self.lr), loss='categorical_crossentropy', metrics=['accuracy'])
    ...
    def __create_network(self, input_shape):
        ...
        x = Dropout(0.3)(x)
        output_layer = Dense(
            3, activation='softmax', name='classification')(x)
    ...
    def train(self, generator, eval_data, steps=100, epochs=10):
        ...
        early_stopping = EarlyStopping(
            patience=50, verbose=1, monitor='val_loss')
        model_checkpoint = ModelCheckpoint(
            './models/{}.hdf5'.format(self.name), save_best_only=True, verbose=1, monitor='val_loss')
        reduce_lr = ReduceLROnPlateau(
            factor=0.01, patience=10, min_lr=1e-9, verbose=1, monitor='val_loss')
